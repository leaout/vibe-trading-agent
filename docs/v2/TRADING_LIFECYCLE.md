...
当前已接通最小决策闭环：后台扫描闭合 K 线并持久化去重候选信号，然后只把锁定策略和信号事实发送给模型，不外发账户余额、持仓、委托或 Broker 信息。模型只返回 `buy/sell/hold + confidence + rationale`；结果写入 `model_decisions_v2` 并投影到 K 线与时间线。每个 `signal_id` 最多调用并保存一次决策。

当前还会在 `decision_audits_v2` 保存每个新候选信号的策略、触发指标、决策资讯、模型请求与结构化响应、运行耗时及模拟执行结果。Web 可点击 K 线信号标记查看该快照。此记录帮助解释单次决策，但尚未保存全部行情历史用于完整重放。
...
当前实现中，页面“启用模拟盘”会把 Session 绑定到系统默认模拟账户并切换为 `paper`。AI BUY/SELL 达到最低置信度后才进入确定性检查：BUY 按策略最大仓位计算数量，SELL 卖出可用数量；成交更新资金、持仓、委托、成交和账本。AI HOLD 不产生委托。行情扫描还会用最新闭合 Bar 更新持仓市价与浮动盈亏。
