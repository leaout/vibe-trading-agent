...
### 4.4 `decision_audits_v2`（当前实现）

每个已持久化候选信号创建一份审计快照，主键为 `signal_id`。此表记录：

- 决策所用结构化策略、信号/K 线指标快照和相关资讯摘要。
- 实际发送的系统提示、用户上下文和响应 Schema。
- 解析后的模型响应、最终动作、置信度、理由、错误状态和耗时。
- 模拟执行结果；若有订单，同时保存订单状态、数量、价格、费用和拒绝原因。

API Key 不进入快照。此快照保存本地当前执行过的决策输入和结果，不包含外部行情源返回的全部历史 K 线，也不保存模型 HTTP 响应包或隐藏推理内容。

### 4.5 `financial_news_v2`
